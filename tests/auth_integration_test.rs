...
use electricity_monitor_backend::{
    ...
    state::AppState,
    routes,
    infrastructure::database::pool::create_pool,
...
async fn create_test_app() -> Router {
    ...
    let redis_pool = electricity_monitor_backend::infrastructure::redis::pool::create_redis_pool(&config.redis)
        .await
        .expect("Redis连接池创建失败");
...
async fn test_admin_token_auth_middleware() {
    ...
    let user_info: serde_json::Value = serde_json::from_str(&body_str)
        .expect("响应体应该是有效的JSON");
