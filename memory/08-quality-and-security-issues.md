...
- 项目当前已具备可执行的依赖审计路径：
  ...
  - `cargo audit`
- 本地开发与测试不应再依赖全局数据库密码环境变量；应通过复制模板并修改本地 `config/default.toml` 的方式提供数据库密码。

## git 历史审计结论
- 当前可见范围内已额外审计 `config/`、`docs/`、`memory/`、`deploy/`、`.github/`、`src/`、`tests/` 的 git 历史。
- 已确认的高风险点是：`config/default.toml` 的 `qq_bot.bearer_token` 在 `9` 个历史提交中出现过非占位符形态的值。
- 其他本次扫描到的主要文件范围内，数据库密码、JWT secret 与 docs 示例未发现明显超出占位符或开发默认值的命中。
- 这项审计不输出历史值本身；`origin/master` 已在 2026-04-03 完成历史重写与强推，后续仍需继续做好协作者清理与凭据轮换。

## 当前执行状态补充
- 仓库当前已经去掉开发环境对全局数据库密码环境变量的依赖，并改成直接修改本地 `config/default.toml`。
- 当前机器上的机器级数据库密码环境变量已清理；若当前桌面线程仍残留进程级环境变量，需要重启宿主应用才能彻底清空继承状态。
- 真实 `QQ/JWT/DB` 凭据轮换仍需外部 secret file 或部署目标访问权；仓库内目前只能记录状态与执行要求，不能伪造“已轮换”结论。
