...
- 默认环境名是 `development`。
- `config/default.toml` 是本地运行时文件，不纳入版本控制；开发和生产都先从模板复制，再在目标环境中补齐真实值。
- 缺少 `config/default.toml` 时，应用会明确报错并提示从模板复制。

## 环境语义

- `development` 环境只能连接本地 PostgreSQL 和本地 Redis。
- `production` 环境允许远端数据库和 Redis，但敏感值必须通过 `*_FILE` 链路注入。
- `APP_ENV` 用于环境校验、生产 fail-fast 和缺失配置提示，不再决定叠加哪个 TOML 文件。
...
- 开发环境要求在复制出来的 `config/default.toml` 中显式填写 `database.password`，不能依赖隐式环境变量。
