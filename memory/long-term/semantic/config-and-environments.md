...
- `[email]` 是后端可复用 SMTP 邮件模块的配置真源，当前不替换 QQ 验证码发送链路。模板固定非敏感默认值：`smtp_host=smtp.qq.com`、`smtp_port=465`、`smtp_user=cogniaegis@qq.com`、`smtp_use_tls=true`、`from_name=CogniAegis`、`timeout_seconds=30`、`max_retries=3`、`retry_delay_seconds=2`。
