...
- `GET /api/public-config` 只暴露前端需要的非敏感运行时配置：可用登录模式、机器人 QQ、管理员 QQ、第三方图形验证码参数和登录验证码参数。
...
- `production` 环境缺少 `jwt.secret_file`、`database.password_file` 或 `qq_bot.bearer_token_file` 时会 fail-fast；如果显式配置了 email SMTP 发送信息，还必须提供 `email.smtp_password_file`。
- `qq_bot.api_url`、`qq_bot.public_qq_number` 与 `qq_bot.bearer_token` 是 QQ 机器人私聊发送链路的运行时必填配置；QQ 登录验证码和 QQ 账号电量通知都会复用这一段 `[qq_bot]` 配置。`qq_bot.api_url` 模板使用 `你的napcat应用URL` 作为占位并用注释给出本地示例，不内置真实部署环境值。`qq_bot.public_qq_number` 不能从 `admin.default_qq_number` 推断，也不能通过自动读取 NapCat 登录信息替代。
- `[email]` 是 SMTP 邮件发送链路配置真源；填写完整 host、user 和有效授权码后会启用邮箱登录和邮箱账号电量通知。模板固定非敏感默认值：`smtp_host=smtp.qq.com`、`smtp_port=465`、`smtp_user=cogniaegis@qq.com`、`smtp_use_tls=true`、`from_name=CogniAegis`、`timeout_seconds=30`、`max_retries=3`、`retry_delay_seconds=2`。
- `email.smtp_password` 是真实 SMTP 授权码，不能写入 tracked 模板或文档；开发调试通过复制后的运行时配置或 `APP__EMAIL__SMTP_PASSWORD(_FILE)` 注入，生产通过 `email.smtp_password_file` / `APP__EMAIL__SMTP_PASSWORD_FILE` 注入 `/run/secrets/app_email_smtp_password`。
- `public_site.domain` 与 `public_site.port` 是通知消息公开访问链接的配置真源，QQ 机器人通知和邮箱通知模板都会使用；模板默认留空，运行时必须配置真实域名和端口，域名只填写域名本身，不包含协议或路径。
