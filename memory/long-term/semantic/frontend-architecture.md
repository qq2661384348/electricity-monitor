...
- `frontend/src/features/auth-login/model/loginIdentity.ts`：登录模式、登录标识规范化、校验和展示标识工具真源；QQ 与邮箱登录 UI 不应在页面里重复硬编码校验细节。
- `frontend/src/features/auth-login/api/authApi.ts`：登录与验证码流程 API 真源。
- `frontend/src/entities/public-config/api/publicConfigApi.ts`：`/api/public-config` 的非敏感运行时配置 API 真源，承接可用登录模式、机器人 QQ、管理员 QQ、验证码和登录验证码参数。
