...
- release `.env` 是生产非敏感运行时值、mimalloc 内存释放参数和 secret file 路径真源，包括 CORS、QQ 机器人发送地址、公开站点域名/端口、管理员 QQ、数据库/JWT/QQ/SMTP secret file 路径；生产配置模板不写入真实 secret 原文。
- release artifact 拆分为 app release 和 infra images；日常发布只需要 app 包，首次部署或 PostgreSQL / Redis 镜像变更时再解压 infra 包。服务器不从外部 registry 拉取镜像，会校验归档 SHA256 与镜像摘要，默认通过 Docker Compose 管理 `postgres`、`redis`、一次性 `migrate` 和 `app`。
- 公开文档只记录通用 release artifact 部署契约；环境专用上传自动化、SSH host alias 和固定服务器路径不作为项目真源。
