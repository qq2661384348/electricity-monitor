...
项目通过运行时 TOML 的 `[captcha]` 和 `[verification]` 控制验证码链路，前端不再硬编码第三方验证码地址、类型、尺寸、难度或登录验证码长度。非敏感参数会由 `GET /api/public-config` 返回给前端。
...
- `verification.code_length` 和 `verification.expire_seconds` 控制登录验证码的长度与 Redis 有效期；验证码 Redis key 会按 `qq` / `email` 渠道隔离。
