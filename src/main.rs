...
async fn main() -> anyhow::Result<()> {
    ...
    if config.room_sync.enabled {
        ...
        if room_count == 0 {
            ...
            use electricity_monitor_backend::domain::services::room_sync::crawler::{
                RoomClient, RoomFetcher,
            };
...
fn spawn_background_services(state: AppState) {
    use electricity_monitor_backend::infrastructure::repositories::{
        UserRepository, UserRoomBindingRepository,
    };
