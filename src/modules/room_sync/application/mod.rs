...
enum RoomSyncJobKind {
    Manual,
    Scheduled,
}

impl RoomSyncJobKind {
    ...
    fn task_name(self) -> &'static str {
        match self {
            Self::Manual => "manual_room_sync",
            Self::Scheduled => "scheduled_room_sync",
        }
    }
}
...
async fn run_room_sync_job(
    ...
    room_path_tree: Arc<RwLock<RoomPathTree>>,
    config: AppConfig,
    kind: RoomSyncJobKind,
) {
    let task_name = kind.task_name();
    tracing::info!(task_name = task_name, module = "room_sync", job_id = %job_id, "开始异步同步任务");

    let new_log = crate::domain::models::NewRoomSyncLog {
        id: Some(job_id),
        sync_type: kind.sync_type().to_string(),
        started_at: Utc::now().naive_utc(),
        status: "running".to_string(),
    };

    if let Err(error) = repository.create_sync_log(new_log).await {
        tracing::error!(task_name = task_name, module = "room_sync", job_id = %job_id, error = %error, "创建同步日志失败");
        return;
    }
    ...
    let client = match RoomClient::new(&config.room_sync.crawler) {
        ...
        Err(error) => {
            let _ = repository
                ...
                .await;
            tracing::error!(task_name = task_name, module = "room_sync", job_id = %job_id, error = %error, "创建爬虫客户端失败");
            return;
        }
    };
    ...
    let room_sync_cache = match RoomSyncCache::new(db_pool.clone()).await {
        ...
        Err(error) => {
            let _ = repository
                ...
                .await;
            tracing::error!(task_name = task_name, module = "room_sync", job_id = %job_id, error = %error, "创建RoomSyncCache失败");
            return;
        }
    };
        ...
        Ok(stats) => {
            ...
            if let Some(fetcher_service) = electricity_fetcher.as_ref() {
                if let Err(error) = fetcher_service.refresh_cache().await {
                    tracing::error!(task_name = task_name, module = "room_sync", job_id = %job_id, error = %error, "刷新电费缓存失败");
                }
            }
            ...
            match repository.find_all_active_path_entries().await {
                ...
                Err(error) => {
                    tracing::error!(task_name = task_name, module = "room_sync", job_id = %job_id, error = %error, "重建路径树失败");
                }
            }
        }
        Err(error) => {
            let _ = repository
                ...
                .await;
            tracing::error!(task_name = task_name, module = "room_sync", job_id = %job_id, error = %error, "同步任务失败");
        }
    }
}
