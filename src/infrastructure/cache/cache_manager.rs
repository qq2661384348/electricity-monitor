...
use super::entity_cache::{CacheConfig, EntityCache, DataLoader};
...
impl DataLoader<i32, Vec<UserRoomBinding>> for BindingLoader {
    ...
    async fn load_batch(&self, keys: &[i32]) -> Result<Vec<(i32, Vec<UserRoomBinding>)>> {
        let all_bindings = self.repository.find_active_bindings_by_roomids(keys).await?;
...
impl CacheManager {
    ...
    pub fn new(
        ...
        let electricity_loader = Arc::new(ElectricityCacheLoader::new(
            RoomRepository::new(db_pool)
        ));
    ...
    pub async fn get_room(&self, roomid: i32) -> Result<Option<Room>> {
        self.room_cache.get_or_load(roomid, self.room_loader.as_ref()).await
    }
    
    /// 批量获取房间
    pub async fn get_rooms(&self, roomids: Vec<i32>) -> Result<Vec<(i32, Option<Room>)>> {
        self.room_cache.get_batch(roomids, self.room_loader.as_ref()).await
    }
    
    /// 获取用户（带缓存）
    pub async fn get_user(&self, user_id: Uuid) -> Result<Option<User>> {
        self.user_cache.get_or_load(user_id, self.user_loader.as_ref()).await
    }
    
    /// 批量获取用户
    pub async fn get_users(&self, user_ids: Vec<Uuid>) -> Result<Vec<(Uuid, Option<User>)>> {
        self.user_cache.get_batch(user_ids, self.user_loader.as_ref()).await
    }
    
    /// 获取房间绑定（带缓存）
    pub async fn get_room_bindings(&self, roomid: i32) -> Result<Option<Vec<UserRoomBinding>>> {
        self.binding_cache.get_or_load(roomid, self.binding_loader.as_ref()).await
    }
    
    /// 批量获取房间绑定
    pub async fn get_rooms_bindings(&self, roomids: Vec<i32>) -> Result<Vec<(i32, Option<Vec<UserRoomBinding>>)>> {
        self.binding_cache.get_batch(roomids, self.binding_loader.as_ref()).await
    }
    
    /// 获取电费（带缓存）
    pub async fn get_electricity(&self, roomid: i32) -> Result<Option<f32>> {
        self.electricity_cache.get_or_load(roomid, self.electricity_loader.as_ref()).await
    }
    
    /// 批量获取电费
    pub async fn get_electricity_batch(&self, roomids: Vec<i32>) -> Result<Vec<(i32, Option<f32>)>> {
        self.electricity_cache.get_batch(roomids, self.electricity_loader.as_ref()).await
    }
...
impl CacheManagerStats {
    ...
    pub fn log_stats(&self) {
        ...
        let total_hits = self.room_stats.l1_hits + self.room_stats.l2_hits
            + self.user_stats.l1_hits + self.user_stats.l2_hits
            + self.binding_stats.l1_hits + self.binding_stats.l2_hits
            + self.electricity_stats.l1_hits + self.electricity_stats.l2_hits;
            
        let total_misses = self.room_stats.l1_misses + self.room_stats.l2_misses
            + self.user_stats.l1_misses + self.user_stats.l2_misses
            + self.binding_stats.l1_misses + self.binding_stats.l2_misses
            + self.electricity_stats.l1_misses + self.electricity_stats.l2_misses;
