...
/// QQ号验证正则表达式（5-20位数字）
pub static QQ_NUMBER_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\d{5,20}$").expect("QQ号正则表达式编译失败")
});

/// 验证码正则表达式（6位数字）
pub static VERIFICATION_CODE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^\d{6}$").expect("验证码正则表达式编译失败")
});
