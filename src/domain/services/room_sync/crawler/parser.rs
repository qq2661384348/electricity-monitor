...
pub fn safe_parse(json_str: &str) -> Result<Value> {
    ...
    // 第一次解析
    let value: Value = sonic_rs::from_str(cleaned).context("第一次JSON解析失败")?;

    // ⭐ 检查是否是双重编码（解析结果是字符串）
    if let Some(inner_str) = value.as_str() {
        tracing::debug!("检测到双重JSON编码，进行第二次解析");
        // 第二次解析
        return sonic_rs::from_str(inner_str).context("双重编码JSON第二次解析失败");
    }
...
pub fn parse_floor_rooms(floor: &Value, base_path: &str) -> Result<Vec<(String, String)>> {
    ...
    for room in rooms {
        let room_name = room["Name"].as_str().context("房间Name字段不存在")?;

        let roomid = room["Value"].as_str().context("房间Value字段不存在")?;
...
pub fn parse_room_tree(json_str: &str) -> Result<Vec<(String, String)>> {
    let root = safe_parse(json_str).context("JSON解析失败")?;

    let campuses = root.as_array().context("根节点应该是数组")?;

    let mut result = Vec::new();

    for campus in campuses {
        let rooms =
            parse_campus_rooms(campus).with_context(|| format!("解析校区失败: {:?}", campus))?;
        result.extend(rooms);
    }
