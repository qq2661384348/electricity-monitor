...
impl RoomSyncService {
    ...
    pub async fn sync(&self) -> Result<SyncStats> {
        ...
        if !to_update.is_empty() {
            let (updated_count, failed_count) = self.batch_update_rooms_transactional(to_update).await?;
    ...
    /// - 使用事务批量执行
    /// - 减少数据库往返次数
    async fn batch_update_rooms_transactional(&self, rooms_to_update: Vec<RoomData>) -> Result<(usize, usize)> {
    ...
    async fn prepare_room_update_ops(&self, room_data: &RoomData) -> Result<RoomUpdateOps> {
        // 1. 查询现有所有路径
        let existing_aggregate = self.repository.find_room_with_all_paths(room_data.roomid)
            .await?
            .ok_or_else(|| AppError::Internal(format!("房间不存在: roomid={}", room_data.roomid)))?;
        ...
        // 3. 检测变化
        let paths_to_add: Vec<String> = new_paths.difference(&existing_paths)
            .filter(|&p| p != &room_data.primary_roompath)
            .cloned()
            .collect();
        
        let paths_to_remove: Vec<String> = existing_paths.difference(&new_paths)
