...
impl NotificationGate {
    ...
    async fn check_recovery_reset(&self, roomid: i32) -> bool {
        ...
        match recovery_map.get(&roomid) {
            Some(recovery_time) => {
                ...
                elapsed
                    >= chrono::Duration::from_std(self.debounce_period)
    ...
    pub async fn mark_notified_persistent(
        ...
        // 2. 持久化到数据库
        binding_repo
            .update_last_notified(user_id, roomid, now)
            .await?;
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match room_repo.find_rooms_recovering().await {
                Ok(recovering_rooms) => {
                    if !recovering_rooms.is_empty() {
                        tracing::debug!(count = recovering_rooms.len(), "检测到恢复中的房间");

                        // 2. 更新恢复状态（持久化）
                        if let Err(e) = gate
                            .update_recovery_state_persistent(&recovering_rooms, &room_repo)
                            .await
                        {
                            tracing::error!("更新房间恢复状态失败: {}", e);
                        }
                    }
                }
                Err(e) => {
                    tracing::error!("查询恢复中的房间失败: {}", e);
                    // 继续执行，不中断任务
                }
            }
            ...
            // 3. 清理已过观察期的状态（持久化）
            match gate
                .cleanup_recovered_persistent(&binding_repo, &room_repo)
                .await
            {
                Ok(cleaned) if cleaned > 0 => {
                    tracing::info!(cleaned = cleaned, "清理了已恢复房间的通知状态（已持久化）");
                }
                Err(e) => {
                    tracing::error!("清理过期状态失败: {}", e);
                }
                _ => {}
            }
...
pub fn spawn_recovery_monitor(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let cleaned = gate.cleanup_recovered().await;
            if cleaned > 0 {
                tracing::info!(cleaned = cleaned, "清理了已恢复房间的通知状态");
            }
