...
- 开始任务前先读 `memory/README.md`，再按任务范围阅读对应长期事实、长期流程或决策目录。
...
- `memory/`：项目记忆主目录；`memory/README.md` 是全局索引入口。
...
- `openspec/` 下的 proposal、requirements、design、worklog、ADR、eval 等可公开工件不能记录本地绝对路径、用户目录、WSL / Windows 挂载路径或私有参考项目源码路径；引用参考实现时只描述可复用能力和必要配置。

## 变更同步要求

- 修改 `memory/` 结构或用途时，同时更新 `memory/README.md`、`memory/long-term/procedural/memory-governance.md` 与 `memory/long-term/semantic/repo-shape-and-agents.md`。
- 修改仓库结构、目录职责或 AGENTS 拓扑时，同步更新 `memory/long-term/semantic/repo-shape-and-agents.md`。
- 修改运行时、环境变量或 secrets 链路时，同步更新 `memory/long-term/semantic/config-and-environments.md`；若改变环境命名 `.toml` 模型，再同步 `memory/decisions/runtime-config-uses-environment-named-toml.md`。
- 修改鉴权、cookie 会话、CORS 或管理员提升规则时，同步更新 `docs/api/API_REFERENCE.md`、`memory/long-term/semantic/auth-session-and-cors.md`；若改变浏览器会话模型，再同步 `memory/decisions/browser-session-uses-memory-access-token-and-cookie-refresh.md`。
- 修改前端工具链、前端工作区规则或前端边界时，同步更新 `frontend/README.md`、`frontend/AGENTS.md`、`memory/long-term/semantic/frontend-architecture.md`、`memory/long-term/semantic/frontend-seams.md`；若改变包管理真源，再同步 `memory/decisions/frontend-package-manager-is-bun.md`。
- 修改后端维护接缝、模块边界或外部 HTTP / 缓存接缝时，同步更新 `memory/long-term/semantic/backend-seams.md`。
- 修改架构热点判断时，同步更新 `memory/long-term/semantic/architecture-hotspots.md`。
- 修改部署链路或 release 包契约时，同步更新 `README.md`、`docs/guides/DOCKER_DEPLOYMENT.md`、`docs/INDEX.md`、`deploy/README.md`、`memory/long-term/procedural/deploy-and-release.md`。
- 修改测试入口、测试 support、CI 门禁或 smoke/readiness 契约时，同步更新 `docs/guides/TESTING.md`、`.github/workflows/ci.yml`、`deploy/smoke.targets`、`memory/long-term/procedural/testing-and-quality-gates.md`。
- 修改质量风险、安全风险或依赖审计基线时，同步更新 `memory/long-term/semantic/quality-and-security-risks.md`。
